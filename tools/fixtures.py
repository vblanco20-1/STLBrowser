...
def box(x, y, z):
    ...
    return [
        tuple(vertices[i] for i in face)
...
if args.large_mb:
    ...
    sphere = surface(
        ...
        48,
        lambda u, v: (
            30 * math.cos(2 * math.pi * u) * math.sin(math.pi * v),
            30 * math.sin(2 * math.pi * u) * math.sin(math.pi * v),
            30 * math.cos(math.pi * v),
        ),
    )
    ...
    torus = surface(
        ...
        48,
        lambda u, v: (
            (30 + 9 * math.cos(2 * math.pi * v)) * math.cos(2 * math.pi * u),
            (30 + 9 * math.cos(2 * math.pi * v)) * math.sin(2 * math.pi * u),
            9 * math.sin(2 * math.pi * v),
        ),
    )
    write(args.directory / "Ring fitting.stl", torus)

    write(
        args.directory / "Two components.stl",
        box(20, 20, 20)
        + [tuple((x + 35, y, z) for x, y, z in t) for t in box(15, 15, 35)],
    )
