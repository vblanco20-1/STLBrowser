...
FileReader::FileReader(const std::filesystem::path& path, Cancel cancel) : p(std::make_unique<Impl>()) {
    p->cancel = std::move(cancel);
    if (p->cancel && p->cancel())
        throw std::runtime_error("Cancelled");
#ifdef _WIN32
    p->file = CreateFileW(path.c_str(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
