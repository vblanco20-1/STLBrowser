...
struct Camera {
    float yaw = -0.8f, pitch = 0.6f, zoom = 1.0f;
    glm::vec2 pan{0};
...
class Renderer {
  public:
    ...
    std::shared_ptr<Mesh> loadMesh(const std::filesystem::path&, uint64_t budget, Cancel, Progress,
                                   Metadata* metadata = nullptr);
    std::shared_ptr<Image> renderView(const std::shared_ptr<Mesh>&, const Camera&, uint32_t, uint32_t,
                                      uint64_t cameraVersion);
  ...
  private:
    ...
    VmaAllocator allocator{};
    VkQueue foreground{}, background{};
    uint32_t family{};
    std::mutex queueMutex, allocationMutex;
    VkSemaphore backgroundTimeline{};
    uint64_t submittedBackground = 0, waitBackground = 0;
    ...
    Frame* current{};
    bool rebuild = false, imguiReady = false;
    VkDescriptorPool descriptorPool{};
    VkSampler sampler{};
    VkPipeline pipeline{};
    VkPipelineLayout pipelineLayout{};
    std::shared_ptr<Image> viewColor, viewDepth, viewDisplayed;
    ...
    void renderChunks(VkCommandBuffer, const Image&, const Image&, const glm::mat4&,
                      const std::vector<std::shared_ptr<Buffer>>&, const std::vector<uint32_t>&, bool clear,
