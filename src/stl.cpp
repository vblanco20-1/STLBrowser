...
Metadata parse(const std::filesystem::path& path, const std::function<void(const Triangle&)>& consume, Cancel cancel,
    Progress progress)
{
...
void streamStl(const std::filesystem::path& p, const Metadata& m, const Chunk& callback, Cancel c, Progress progress,
    size_t chunkSize)
{
