...
glm::mat4 Camera::matrix(float aspect) const
{
    ...
    float h = 0.92f * zoom;
    float w = h * std::max(aspect, 0.05f);
    if (aspect < 1) {
        h /= aspect;
        w = 0.92f * zoom;
    }
...
void barrier(VkCommandBuffer cmd, VkImage image, VkImageLayout oldLayout, VkImageLayout newLayout,
    VkImageAspectFlags aspect, VkPipelineStageFlags2 src, VkAccessFlags2 srcAccess, VkPipelineStageFlags2 dst,
...
Renderer::Renderer(SDL_Window* w, bool validation, bool singleQueue)
    : window(w)
{
    unsigned count = 0;
    ...
    auto selected = vkb::PhysicalDeviceSelector { ir.value() }
                        ...
                        .select();
    if (!selected) {
        throw std::runtime_error("A Vulkan 1.3 GPU with dynamic rendering and timeline semaphores is required: "
...
std::shared_ptr<Buffer> Renderer::buffer(VkDeviceSize bytes, VkBufferUsageFlags usage, bool host)
{
    ...
    if (bytes > deviceBudget || allocatedBytes() + bytes > deviceBudget) {
        throw std::runtime_error("GPU allocation budget reached; close the current model or raise the budget");
    }
    ...
    check(vmaCreateBuffer(allocator, &info, &allocation, &b->buffer, &b->allocation, &details), "Allocate buffer");
    b->mapped = details.pMappedData;

    return b;
}

std::shared_ptr<Image> Renderer::image(uint32_t width, uint32_t height, VkFormat format, VkImageUsageFlags usage)
{
    std::lock_guard lock(allocationMutex);
    ...
    v.format = format;
    v.subresourceRange
        = { VkImageAspectFlags(format == VK_FORMAT_D32_SFLOAT ? VK_IMAGE_ASPECT_DEPTH_BIT : VK_IMAGE_ASPECT_COLOR_BIT),
              0, 1, 0, 1 };
...
void Renderer::createPipeline()
{
    ...
    VkShaderModule vertex = shader(device, "mesh.vert.spv");
    VkShaderModule fragment = shader(device, "mesh.frag.spv");
...
void Renderer::renderChunks(VkCommandBuffer cmd, const Image& color, const Image& depth, const glm::mat4& matrix,
    const std::vector<std::shared_ptr<Buffer>>& chunks, const std::vector<uint32_t>& counts, bool clear, size_t first,
    size_t end)
{
    barrier(cmd, color.image, clear ? VK_IMAGE_LAYOUT_UNDEFINED : VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
        ...
        clear ? 0 : VK_ACCESS_2_COLOR_ATTACHMENT_WRITE_BIT, VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT,
    ...
    barrier(cmd, depth.image, clear ? VK_IMAGE_LAYOUT_UNDEFINED : VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL,
        ...
        clear ? VK_PIPELINE_STAGE_2_NONE
...
Thumbnail Renderer::thumbnail(const std::filesystem::path& path, Cancel cancel, Progress progress)
{
    ...
    auto geometry = buffer(16384 * 36, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, false);
    streamStl(
        path, out.metadata,
        [&](std::span<const glm::vec3> vertices) {
            ...
            check(vmaFlushAllocation(allocator, staging->allocation, 0, vertices.size_bytes()), "Flush vertices");
            context.begin();
            ...
            renderChunks(
                context.command, *out.image, *depth, matrix, { geometry }, { uint32_t(vertices.size()) }, first, 0, 1);
            context.finish();
            first = false;
        },
        cancel,
    ...
    vkCmdCopyImageToBuffer(
        context.command, out.image->image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, readback->buffer, 1, &copy);
    barrier(context.command, out.image->image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_ASPECT_COLOR_BIT, VK_PIPELINE_STAGE_2_TRANSFER_BIT,
    ...
    check(vmaInvalidateAllocation(allocator, readback->allocation, 0, VK_WHOLE_SIZE), "Read thumbnail pixels");
...
std::shared_ptr<Image> Renderer::uploadImage(
    const std::vector<uint8_t>& pixels, uint32_t width, uint32_t height, Cancel cancel)
{
    if (cancel && cancel()) {
        throw std::runtime_error("Cancelled");
    }
    ...
    auto out
        = image(width, height, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    ...
    vkCmdCopyBufferToImage(
        context.command, staging->buffer, out->image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copy);
    barrier(context.command, out->image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
        VK_IMAGE_ASPECT_COLOR_BIT, VK_PIPELINE_STAGE_2_TRANSFER_BIT, VK_ACCESS_2_TRANSFER_WRITE_BIT,
...
std::shared_ptr<Mesh> Renderer::loadMesh(
    const std::filesystem::path& path, uint64_t budget, Cancel cancel, Progress progress, Metadata* metadata)
{
    ...
    if (out->metadata.geometryBytes() > budget) {
        throw std::runtime_error("Exact geometry needs "
            + std::to_string((out->metadata.geometryBytes() + 999999) / 1000000) + " MB; viewer limit is "
            + std::to_string(budget / 1000000) + " MB. Raise the budget and retry.");
    }

    if (out->metadata.geometryBytes() + allocatedBytes() + 64000000 > deviceBudget) {
        throw std::runtime_error("Not enough overall GPU budget for this model; raise the total budget and retry");
    }
    ...
    streamStl(
        ...
        [&](std::span<const glm::vec3> vertices) {
            ...
            auto geometry = buffer(
                vertices.size_bytes(), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, false);
            std::memcpy(staging->mapped, vertices.data(), vertices.size_bytes());
            check(vmaFlushAllocation(allocator, staging->allocation, 0, vertices.size_bytes()), "Flush model vertices");
            context.begin();
...
void Renderer::use(const std::shared_ptr<Image>& img)
{
    ...
    if (!img->descriptor) {
        img->descriptor = ImGui_ImplVulkan_AddTexture(sampler, img->view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    }
...
std::shared_ptr<Image> Renderer::renderView(
    const std::shared_ptr<Mesh>& mesh, const Camera& camera, uint32_t width, uint32_t height, uint64_t version)
{
    if (!mesh) {
        return {};
    }
    ...
    if (viewVersion != version || viewMesh != mesh || !viewColor || viewColor->width != width
        ...
        viewColor = image(
            width, height, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
    ...
    if (viewChunk < mesh->chunks.size()) {
        ...
        renderChunks(current->command, *viewColor, *viewDepth, camera.matrix(float(width) / height), mesh->chunks,
            mesh->counts, viewChunk == 0, viewChunk, end);
        viewChunk = end;
...
bool Renderer::drawFrame()
{
    ImGui::Render();
    uint32_t index = 0;
    auto acquire = vkAcquireNextImageKHR(device, swapchain, UINT64_MAX, current->acquired, VK_NULL_HANDLE, &index);
    ...
    barrier(current->command, swapImages[index], VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
        VK_IMAGE_ASPECT_COLOR_BIT, VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT, 0,
        VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT, VK_ACCESS_2_COLOR_ATTACHMENT_WRITE_BIT);
    ...
    if (!capturePath.empty()) {
        ...
        vkCmdCopyImageToBuffer(
            current->command, swapImages[index], VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, capture->buffer, 1, &region);
        ...
        barrier(current->command, swapImages[index], VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
            VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, VK_IMAGE_ASPECT_COLOR_BIT, VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT,
    ...
    if (capture) {
        check(vkWaitForFences(device, 1, &current->fence, VK_TRUE, UINT64_MAX), "Explicit screenshot completion");
        check(vmaInvalidateAllocation(allocator, capture->allocation, 0, VK_WHOLE_SIZE), "Screenshot readback");
