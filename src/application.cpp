...
void metadataText(const Metadata& m)
{
    ...
    if (m.degenerate) {
        ImGui::TextColored(
            { 1, .7f, .3f, 1 }, "%llu degenerate triangles skipped", static_cast<unsigned long long>(m.degenerate));
    }
...
Application::Application(Options o)
    : options(std::move(o))
{
    SDL_SetMainReady();
...
void Application::requestTree(TreeNode& node)
{
    ...
    directoryWorkers.submit(0, [this, target, p, generation] {
        try {
            target->result = enumerate(p, false, [this, generation] {
                return stopping || rootGeneration != generation;
            });
...
void Application::selectFolder(const std::filesystem::path& p)
{
    ...
    directoryWorkers.submit(-1, [this, target, p, generation, descendants] {
        try {
            target->result = enumerate(p, descendants, [this, generation] {
                return stopping || browseGeneration != generation;
            });
        } catch (const std::exception& e) {
            target->result.error = e.what();
        }
    ...
    workers.submit(priority, [this, j, pixels, oldMetadata] {
        ...
        try {
            ...
            if (pixels) {
                ...
                j->thumbnail = renderer->thumbnail(j->file.path, cancel, [j](float p) {
                    j->progress = p;
                });
            }
...
void Application::drawGrid()
{
    ...
    if (result.partial) {
        ImGui::TextColored(
            { 1, .7f, .3f, 1 }, "Partial listing: 100,000 directory entries reached. Select a smaller subtree.");
    }
    ...
    ImGui::TextDisabled("%zu files  |  %s", filtered.size(), recursive ? "Including subfolders" : "Current folder");
    ImGui::Separator();
    if (filtered.empty()) {
    ...
    float height = width + 3 * ImGui::GetTextLineHeightWithSpacing() + ImGui::GetFrameHeightWithSpacing() + 12;
        ...
        for (int row = clip.DisplayStart; row < clip.DisplayEnd; ++row) {
            ...
            for (int col = 0; col < columns; ++col) {
                ...
                if (texture) {
                    ...
                    ImGui::Image(reinterpret_cast<ImTextureID>(found->second.image->descriptor), { width, width });
                } else {
                    ...
                    draw->AddText({ start.x + (width - size.x) / 2, start.y + width / 2 }, IM_COL32(123, 147, 166, 255),
                        text.c_str());
                }
                ...
                if (ImGui::IsItemHovered()) {
                    draw->AddRect(start, { start.x + width, start.y + width }, IM_COL32(91, 190, 179, 255), 7, 0, 2);
                    ImGui::BeginTooltip();
                ...
                draw->PushClipRect(
                    textPosition, { textPosition.x + width, textPosition.y + ImGui::GetTextLineHeight() }, true);
...
void Application::drawViewer()
{
    ...
    auto image
        = renderer->renderView(model, camera, uint32_t(std::max(size.x, 64.0f)), uint32_t(size.y), cameraVersion);
    if (image) {
        renderer->use(image);
        ImGui::GetWindowDrawList()->AddImage(
            reinterpret_cast<ImTextureID>(image->descriptor), position, { position.x + size.x, position.y + size.y });
    }
...
void Application::settings()
{
    ...
    if (ImGui::Begin("Memory and rendering", &showSettings)) {
        ...
        ImGui::TextDisabled(
            "%s", renderer->separateQueue ? "Separate background graphics queue" : "Serialized single-queue fallback");
    }
...
{
    if (!prebuild || paused || viewerFile || !prebuildListing || !prebuildListing->done.load(std::memory_order_acquire)
        || active.size() >= 4) {
        return;
    }
...
void Application::drawUI()
{
    ...
    if (ImGui::InputTextWithHint("##path", "Local folder or UNC network path", pathText.data(), pathText.size(),
    ...
    if (ImGui::Button(prebuild ? "Stop prebuild" : "Prebuild tree") && !root.empty()) {
        ...
        if (prebuild) {
            ...
            directoryWorkers.submit(5, [this, target, p] {
                try {
                    target->result = enumerate(p, true, [this] {
...
int Application::run()
{
    ...
    bool running = true;
    bool captured = false;
    bool viewerCaptured = false;
    int exerciseStage = 0;
    uint64_t peakRAM = 0;
    uint64_t peakVulkan = 0;
    ...
    std::cout << "Rendered " << renderer->frames << " frames; validation errors: " << renderer->validationErrors
              << "; peak process bytes: " << peakRAM << "; peak Vulkan bytes: " << peakVulkan
              << "; frame p95 ms: " << (frameTimes.empty() ? 0 : frameTimes[size_t((frameTimes.size() - 1) * .95)])
...
int Application::selfTest()
{
    ...
    try {
        renderer->thumbnail(options.root,
            [] {
                return true;
            },
            {});
    ...
    uint64_t peakVulkan = renderer->allocatedBytes();
    uint64_t peakRAM = processMemory();
    for (int i = 0; i < 10000; ++i) {
        renderer->beginFrame();
        renderer->use(i % 2 ? thumb.image : cachedImage);
        ImGui::Begin("Integration test");
        ImGui::Image(reinterpret_cast<ImTextureID>((i % 2 ? thumb.image : cachedImage)->descriptor), { 320, 320 });
        ...
        if (i >= 12 && renderer->viewProgress == 1 && !options.screenshot.empty()) {
            renderer->requestCapture(
                options.screenshot.parent_path() / (options.screenshot.stem().string() + "_full.ppm"));
        }
    ...
    std::cout << "Integration passed: " << thumb.metadata.triangles << " triangles, " << changed << " shaded pixels, "
              << renderer->validationErrors << " validation errors; thumbnail seconds: " << thumbnailSeconds
              << "; total seconds: "
