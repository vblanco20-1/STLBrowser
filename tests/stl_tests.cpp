...
void binary(const std::filesystem::path& path, uint32_t count = 4, bool invalid = false) {
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        for (int j = 0; j < 3; ++j)
            for (int a = 0; a < 3; ++a)
                number(out, invalid && i == 0 && j == 0 && a == 0 ? std::numeric_limits<float>::infinity()
                                                                  : v[faces[i % 4][j]][a]);
        out.put(0);
        out.put(0);
    }
...
int main(int argc, char** argv) {
    try {
        ...
        binary(bad, 4, true);
        fails([&] { si::inspectStl(bad); });
        binary(bad);
        std::filesystem::resize_file(bad, 100);
        fails([&] { si::inspectStl(bad); });
        binary(bad, 0);
        fails([&] { si::inspectStl(bad); });
        fails([&] { si::inspectStl(p, [] { return true; }); });
        binary(bad);
        {
        ...
        size_t streamed = 0;
        si::streamStl(p, large, [&](auto v) { streamed += v.size() / 3; }, {}, {}, 7);
